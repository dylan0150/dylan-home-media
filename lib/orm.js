...
class ORM extends EventEmitter {
        ...
        ;(async () => {
            ...
            try {
                ...
                ;`,{
                    ...
                    column: { value: name, identifier: true }
                })
            } catch (e) {
                err = e
            } finally {
    ...
    next() {
        if (this.queue.length === 0) return this.busy = false
        return this.queue.shift().resolve()
    }

    /**
     * Returns a promise that is resolved at the end of the current queue, after all current statements are resolved
     */
    async wait() {
        if (!this.busy) {
            this.busy = true
